#!/usr/bin/env bash
# Suite del guard. Los payloads los serializa PHP con json_encode: el escapado (rutas Windows
# incluidas) es el mismo que envía Claude Code.
...
t(){ printf '%s' "$2" | bash "$G" >/dev/null 2>&1; got=$?
  ...
  printf '%s %-34s esperado=%s obtenido=%s\n' "$r" "$1" "$3" "$got"; }
D="doctrine:database:drop"; U="doctrine:schema:update"; E=".env.local"; J="config/jwt"
t "drop de BD"                 "$(mk Bash command "php bin/console $D --force")" 2
t "schema update"              "$(mk Bash command "php bin/console $U --force")" 2
t "force push"                 "$(mk Bash command "git push --force origin main")" 2
t "push -f"                    "$(mk Bash command "git push -f origin main")" 2
...
t "mencionar schema:update en texto" "$(mk Bash command "grep -rn '$U' docs/")" 0
t "crear entidad (permitido)"  "$(mk Write file_path 'C:\proj\backend\src\Entity\Product.php')" 0
t "escribir migracion"         "$(mk Write file_path '/c/proj/backend/migrations/Version20260101.php')" 2
t "editar migracion Windows"   "$(mk Edit file_path 'C:\proj\backend\migrations\Version20260101.php')" 2
t "escribir .env.local"        "$(mk Write file_path '/c/proj/backend/.env.local')" 2
